namespace DurablePuppeteer
{
    ...
    public static class WorkerOrchestrators
    {
            ...
            )
        {
            ...
            {
                var page = batch[i];
                var worker = ctx.CallActivityAsync<WorkerResult>("RetrievePageContent", new Tuple<string, string, int, string, int>(query, cookiestext, page, configstring, total));
            ...
            using (var cts = new CancellationTokenSource())
            {
                ...
                if (winner == longtask)
                {
                    ...
                    foreach (var wr in results)
                    {
                        ...
                        await ctx.CallActivityAsync("StorePageContent", new Tuple<string,string,int,int>(wr.html,wr.suburb,wr.totalpages,wr.page));
                        if (!ctx.IsReplaying)
                        {
    ...
    public class WorkerResult
    {
        ...
        public string suburb { get; set; } // Search query
    ...
    public static class Constants
    {
        ...
        public static readonly string BrowserWSEndpoint = System.Environment.GetEnvironmentVariable("RemoteBrowserEndpoint", EnvironmentVariableTarget.Process);
        internal static readonly string ServiceBusConenction = System.Environment.GetEnvironmentVariable("ServiceBusConnection", EnvironmentVariableTarget.Process);
        internal static readonly string ServiceBusQueue = System.Environment.GetEnvironmentVariable("ServiceBusQueue", EnvironmentVariableTarget.Process);
    }
}
